...
def create_predictor(args, mode, logger):
    ...
    if args.use_onnx:
        ...
        target_models = ['ch_PP-OCRv4_det_infer.onnx', 'ch_PP-OCRv4_rec_infer.onnx']
        if any(model_file_path.endswith(model) for model in target_models):
            model_file_path = model_file_path.replace(".onnx", "_linux_aarch64.om")
        if not os.path.exists(model_file_path):
            raise ValueError("not find model file path {}".format(
                model_file_path))
        #sess = ort.InferenceSession(model_file_path)
        sess = InferSession(0, model_file_path)
