...
class LandmarkRunner(object):
    ...
    def __init__(self, **kwargs):
        ...
        print('------------------------------------')
        ckpt_path = '/home/HwHiAiUser/LivePortrait/pretrained_weights/liveportrait/landmark.om' 
        self.session = InferSession(int(device_id), ckpt_path)

    def _run(self, inp):
        #out = self.session.run(None, {'input': inp})
        out = self.session.infer([inp])
        return out
